...
int perceptron_trace(int argc, char* argv[]) {
  ...
  if (argc == 3 && std::string(argv[1]) == "model") {
    ...
    std::wcout << "Output templates and features from an mtx file.\n";
    std::wcout << argv[0] << " path <binary model file> <untagged> <tagged>\n";
    ...
    std::wcout << argv[0] << " tag\n";
    std::wcout << "Tag STDIN tracing the whole process. Useful for "
               << "interactively designing feature sets.\n";
  }
  return 0;
}
