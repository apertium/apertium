...
Transfer::processChunk(xmlNode *localroot)
{
  ...
  if(!caseofchunk.empty() && !dictionary_case)
  {
    ...
    {
      I18n(APER_I18N_DATA, "apertium").error("APER1135", {}, {}, true);
    }
  }
  else
  {
...
Transfer::processLet(xmlNode *localroot)
{
    ...
    switch(ti.getType())
    {
      ...
      case ti_clip_sl:
        if (checkIndex(leftSide, ti.getPos(), lword)) {
          ...
          if (!match && trace)
          {
            I18n(APER_I18N_DATA, "apertium").error("APER1136", {"tag", "line"},
  ...
  else if(!xmlStrcmp(leftSide->name, (const xmlChar *) "clip"))
  {
    ...
    if (pos >= lword) {
      I18n(APER_I18N_DATA, "apertium").error("APER1137", {}, {}, false);
      return;
    }
    if (word[pos] == 0) {
      I18n(APER_I18N_DATA, "apertium").error("APER1138", {}, {}, false);
      return;
    }
...
Transfer::processCallMacro(xmlNode *localroot)
{
  ...
  if(npar > 0)
  {
    ...
    for (auto i : children(localroot)) {
      if (idx >= npar) {
        I18n(APER_I18N_DATA, "apertium").error("APER1139", {"line"}, {i->line}, false);
        return;
      }
